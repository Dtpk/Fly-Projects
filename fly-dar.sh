...
if [ ! -d "$VENV_DIR" ]; then
    ...
    pip install --no-cache-dir --upgrade pip numpy pygame
    deactivate
    echo "[+] Setup complete!"
else
...
radar_angle = 0.0
classic_blips = []
modes_list = ["HALO_RADAR", "BRAIN_SCAN", "RADAR"]
mode_index = 0
...
while running:
    ...
    if current_mode == "HALO_RADAR":
        ...
        rotation_angle_y += 0.012
        cos_y, sin_y = np.cos(rotation_angle_y), np.sin(rotation_angle_y)

        projected_coords = []
        center_3d_x, center_3d_y = center[0], center[1]

        for pt in base_nodes_3d:
            x_rot = pt[0] * cos_y - pt[2] * sin_y
            z_rot = pt[0] * sin_y + pt[2] * cos_y
            y_rot = pt[1]
        ...
        for p in neural_pulses[:]:
            p["progress"] += p["speed"]
            if p["progress"] >= 1.0:
                neural_pulses.remove(p)
                continue
            p1 = np.array([projected_coords[p["from"]][0], projected_coords[p["from"]][1]])
            p2 = np.array([projected_coords[p["to"]][0], projected_coords[p["to"]][1]])
            curr_pos = p1 + (p2 - p1) * p["progress"]
            pygame.draw.circle(screen, (255, 255, 255), (int(curr_pos[0]), int(curr_pos[1])), max(2, int(3 * min_scale)))

        if len(neural_pulses) > 40:
            neural_pulses.pop(0)
