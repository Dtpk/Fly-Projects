...
If your wondering why not load the full connectome thing? It would consume more then 102gb of ram or more which is why lots of other projects just use a section like I been doing , updated the sparce method is introduced think of it as getting rid of empty space but it is still all the fruit-flys connections and the whole thing running right now only the fly-dar-learning has it and the visualizer for seeing all the neurons is laggy so might tweak as I go but your not suppose to load it all anyway it benefits running one section over all but it is a neat gimmick and does work.
