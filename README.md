...
A heads up I am working on a selector or load as needed method right now it grabs a random 64 nodes for the file manage and the fly-dar the fly-dar will get a fly-dar-learning version which will have a selector letting you use more select specific sections "etc" and it will learn and adjust the weights not just be static not sure if this will improve it more but it a start.
